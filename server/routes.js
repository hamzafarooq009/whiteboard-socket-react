...
router.get("/whiteboards", async (req, res) => {
  ...
  try {
    ...
    const whiteboards = await Whiteboard.find({
      ...
      .exec(); // Optionally populate owner details
...
router.get("/whiteboards/:id", async (req, res) => {
  ...
  try {
    ...
    if (!whiteboard.owner.equals(req.user._id)) {
      return res
        .status(403)
        .send("User is not authorized to view this whiteboard");
    }
    res.send(whiteboard);
  } catch (err) {
    console.error(err);
...
router.put("/whiteboards/:id", async (req, res) => {
  ...
  try {
    ...
    if (!whiteboard.owner.equals(req.user._id)) {
      return res
        .status(403)
        .send("User is not authorized to update this whiteboard");
    }
...
router.delete("/whiteboards/:id", async (req, res) => {
  ...
  try {
    ...
    if (!whiteboard.owner.equals(req.user._id)) {
      return res
        .status(403)
        .send("User is not authorized to delete this whiteboard");
    }
...
router.put("/whiteboards/:id/share", async (req, res) => {
  ...
  try {
    ...
    if (!whiteboard.owner.equals(req.user._id)) {
      return res
        .status(403)
        .send("User is not authorized to share this whiteboard");
    }
